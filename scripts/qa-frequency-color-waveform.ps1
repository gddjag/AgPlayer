...
function New-TargetContext {
    param([string]$Label, [string]$Root, [string]$BuildDirectoryArgument, [string]$FixtureArgument, [string]$ProbeSource)
    ...
    return [pscustomobject]@{
        ...
        runtimePaths = @($qtBin, $vcpkgBin); commit = (& git -C $resolvedRoot rev-parse HEAD).Trim(); appTreeBytes = Get-AppTreeBytes -BuildRoot $resolvedBuild
    }
}
...
$candidate = New-TargetContext -Label "candidate" -Root $repoRoot -BuildDirectoryArgument $BuildDirectory -FixtureArgument $Fixture -ProbeSource $probeSource
...
try {
    if ($BaselineOnly) {
        ...
        $document = [ordered]@{ schema = "agplayer.frequency-color-waveform.qa.v4"; kind = "baseline"; commit = $candidate.commit; command = $MyInvocation.Line; build = $candidate.config; fixture = $candidate.fixture; fixtureSha256 = $candidate.fixtureSha256; probeSource = $candidate.probeSource; probeSourceSha256 = $candidate.probeSourceSha256; probeExecutable = $candidate.probeExecutable; probeExecutableSha256 = $candidate.probeExecutableSha256; appTreeBytes = $candidate.appTreeBytes; probeIterations = $ProbeIterations; pairCount = $PairCount; measurementAffinityMask = $measurementAffinityMask; normalWaveform = $summary; result = "PASS" }
    ...
    $baselineDocument = [ordered]@{ schema = "agplayer.frequency-color-waveform.qa.v4"; kind = "baseline"; commit = $baseline.commit; command = $MyInvocation.Line; measurementPriorityClass = "High"; measurementAffinityMask = $measurementAffinityMask; pairCount = $PairCount; pairSeed = $PairSeed; build = $baseline.config; dependencies = $baseline.dependencies; fixture = $baseline.fixture; fixtureSha256 = $baseline.fixtureSha256; probeSource = $baseline.probeSource; probeSourceSha256 = $baseline.probeSourceSha256; probeExecutable = $baseline.probeExecutable; probeExecutableSha256 = $baseline.probeExecutableSha256; appTreeBytes = $baseline.appTreeBytes; probeIterations = $ProbeIterations; normalWaveform = $baselineNormal; paired = [ordered]@{ cold = $coldPaired; hot = $hotPaired }; result = "RECORDED" }
    ...
    $stressTest = Join-Path $candidate.buildRoot "tests/frequency_color_waveform_stress_test.exe"; if (-not (Test-Path -LiteralPath $stressTest -PathType Leaf)) { throw "Frequency stress executable is missing: $stressTest" }
