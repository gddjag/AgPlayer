...
// Nominal non-transient 5/10/20 ms frames therefore use 240/480/960 bins.
...
constexpr std::size_t framesPerAnchor = 12, anchorCount = 4;
...
struct CeltFramingProbe::Impl final {
    const std::size_t hop, size, excerptSize, lowBins;
    int channels;
    std::uint64_t cursor = 0;
    std::array<std::uint64_t, anchorCount> starts{};
    std::size_t current = 0, measured = 0;
    ...
    Impl(int channelCount, std::uint64_t total, std::size_t frameSamples)
        : hop(frameSamples), size(2 * hop), excerptSize(size + framesPerAnchor * hop),
          lowBins(hop * 3 / 4), channels(channelCount), input(size), output(size)
    {
    ...
    void measure(const std::atomic_bool& cancel)
    {
        ...
        for (std::size_t shift = 0; shift < hop; ++shift) {
            ...
            std::array<double, framesPerAnchor> fullFrames{}, lowFrames{};
            for (std::size_t frame = 0; frame < framesPerAnchor; ++frame) {
                ...
                for (std::size_t k = 0; k < hop; ++k) {
                    fullDb -= 20.0 * std::log10(std::max(std::abs(coefficients[k]) / fullRms, normalizedCoefficientFloor));
                    if (k < lowBins)
                        lowDb -= 20.0 * std::log10(std::max(std::abs(coefficients[k]) / lowRms, normalizedCoefficientFloor));
                }
                fullFrames[frame] = fullDb / hop;
                lowFrames[frame] = lowDb / lowBins;
            }
            const auto phase = (starts[current] + shift) % hop;
            full[phase] = median(fullFrames);
            low[phase] = median(lowFrames);
        }
...
CeltFramingProbe::CeltFramingProbe(int rate, int channels, std::uint64_t total, std::size_t frameSamples)
{
    if (rate == 48000 && channels > 0 && channels <= 64
            && (frameSamples == 240 || frameSamples == 480 || frameSamples == 960))
        impl_ = std::make_unique<Impl>(channels, total, frameSamples);
}
