...
- 现有 `vocal_protection` 被标注为 experimental，不能仅凭 API 名称宣称真实 Formant Preservation。只有完成受控音频 A/B、算法路径和输出差异验证后，才显示参考图中的“人声保真 / Formant保护”；否则隐藏该行并让保持音调区自然收缩。
...
- D. 保持音调：变速时保持音调；Formant 行按真实能力条件显示。
- E. 导出设置：格式、采样率、位深、声道、比特率、输出目录和蓝色“导出音频”。

### 14.3 工具栏语义

- 导入音频、保存工程、选择、分割、删除、裁剪、复制、粘贴、淡入、淡出、静音片段和清除必须连接真实命令或在状态不满足时正确禁用。
- 参考图中的“降噪”与最新文档“只有导出音频/拖出片段才允许生成新音频”的合同冲突；当前降噪会生成替换源，因此本版本从工具栏和编辑器专用代码中删除，不留空壳按钮，也不以新的实时算法替代。
...
- 不显示参考图 Demo 文件、虚假设备、虚假 BPM、虚假电平或错误的选区时长。
- 所有核心控件必须工作；非核心视觉控件也不能伪装成功。
...
- QML：工具顺序、1672×941 布局合同、快捷键、真实 enabled 状态、条件 Formant、空态和小尺寸可用性。
...
- 参考图中的错误选择时长、Demo 数据和未验证 Formant 不作为像素复刻目标。
