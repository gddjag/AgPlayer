...
| 自定义 Accent / Highlight、10 个预设、原生 Picker、跟随/独立模式 | C++ `ThemeManager`、设置事务、四语设置页；Light/Dark 对比度测试、10 预设和黑白黄 UI 矩阵、真实 WAV 播放中切换 | 已验证 |
| 完美匹配 Windows/macOS/Linux/HarmonyOS 默认主题与双色图标 | Windows 有主题基础；非 Windows 平台未构建和视觉验收 | 待人工 |
| 毛玻璃设置 | 当前仅透明度效果，不是系统毛玻璃 | 未实现 |
| 中文、英文、泰语、越南语 | 4 个 TS/QM 目录；各目录完整、未完成 0；最终 4 语言 × 3 主题 × 10 状态共 120 个 UI 状态完成截图校验 | 已验证 |
...
| Windows 独立 EXE 安装包 | MSVC Release 全量 107/107；打包目录真实 WAV 冒烟通过；`AgPlayer-Setup-1.0.0-x64.exe` 已复制桌面并核对 SHA-256 | 已验证 |
