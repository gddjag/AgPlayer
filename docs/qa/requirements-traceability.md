...
| 歌曲切换淡入淡出 0/200/500 ms | 未实现播放队列双流交叉淡化 | 未实现 |
| 自动匹配每首歌曲采样率 | 首曲初始化设备；后续曲目解码到固定会话采样率，无设置开关 | 未实现 |
...
| 原始设置清单全部可交互 | 输出设备和独占已接通；切歌淡化、自动采样率等仍缺失 | 未实现 |
...
| 中文、英文、泰语、越南语 | 4 个 TS/QM 目录；457 条完整；翻译与 80 张 UI 矩阵测试 | 已验证 |
