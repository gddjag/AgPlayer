...
## 1.1.2 后续三项调整（基线 main@194667d）

- 柱体数量使用相对密度 50%–200%，默认 125%，支持滑条与 ±5% 按钮并持久化。画质及自动降级继续限制实例上限；不增加渲染 pass。真实 Direct3D11 测试默认柱数 5681→7089（+24.8%），数量变化不重建管线资源。
- 滚动工具栏按标题、控件的实际总高度居中，而非固定空容器高度；BPM 与分组下拉框统一 28 DIP。1180/1386/1600 宽度运行布局测试通过，窄窗保留换行。
- 经典播放器三行信息改用自然高度等距排列并与封面中心对齐；评分 19→17 DIP，元数据芯片缩小内边距，保持字体不变。覆盖 266/360/500 高度及最小窗口回归。
- 验证：主窗口、滚动主题、翻译相关 CTest 5/5；沉浸 QML 35/35；地形状态 49/49；GPU 26/26；controller/item CTest 通过。构建及 diff whitespace 检查通过。
- 已核对 `build/qa/round2-classic*.png`、`round2-rolling*.png`、`round2-density-panel.png`；`round2-immersive.png` 使用合成频段驱动验证视觉，不作为真实音乐节拍匹配证据。
- 本次质感改善限定于更密的圆形柱体细节，未宣称达到 C4D/UE5 画质。未进行全量 Debug/Release 回归，未新增安装包。
