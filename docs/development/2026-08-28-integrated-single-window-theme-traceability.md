...
## 2026-08-29 第五轮波形竖边与底栏间距修复

| 修复要求 | 根因与最小实现 | 验证 |
|---|---|---|
| 波形不再出现遮挡竖边 | Integrated 原先用第二个完整 `WaveformItem` 按播放位置矩形裁切，裁切边缘在高振幅处形成竖边；改为唯一波形画布直接绑定 `positionMs` 进行进度着色 | TDD 断言唯一画布、播放位置绑定、1672×941 播放截图 |
...
## 2026-08-29 第七轮恢复已播放进度色

| 修复要求 | 根因与最小实现 | 验证 |
|---|---|---|
| 恢复单窗口已播放进度色 | 第六轮误把进度色本身当成裁切伪影，令主波形 `position` 固定为 0；恢复为共享播放位置，同时继续由同一个 `WaveformItem` 完成着色 | `qml_integrated_theme_test` 在 25,000ms 断言 `position` 与 `cursorPosition` 都跟随播放位置 |
| 不重新引入重复裁切层 | 不恢复已删除的第二个波形项；继续断言 `integratedPlayedWaveform` 不存在，并保留共享渲染器首列描边缩进 | QML 单画布断言、`waveform_item_test`、1672×941 v7 播放截图 |

本轮只恢复必要绑定和测试契约；没有新增渲染层、依赖、缓存或线程。
