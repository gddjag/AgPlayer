...
- 标签选中保持标签色填充，不显示白色内圈；专业模式副标题与 16px 星星居中。
- 分组、视窗与 BPM 框统一 62×24px；公共输入框背景跟随实际高度，BPM 减少内边距，保留错误提示区域。
- 三种播放器共享音量滑条去掉隐藏内边距，百分比间隔 4px；迷你收藏移至标题后。
- 分离卡片在后台探测时可立即切换；按模型、文件与运行时/驱动身份复用探测结果，显式探测可强制刷新。
- 异模型下载与已安装模型分离可并行；同模型和共享运行时写入仍受保护。后台验证占用时排队启动，不丢失分离上下文；取消排队启动不取消独立下载。
- 新歌单自动展开导航并滚入可见区；资源点击由稳定视图处理，避免模型重置销毁手势，同时防止错选替换节点和展开箭头误触发过滤。

验证：Release 构建通过。分离控制器 81 通过、2 opt-in 跳过（`feedback-controller-final.txt`），最后排队配置保护定向 6/6（`feedback-controller-guard.txt`）；分离 QML 46 通过、2 跳过；专业模式 36/36、迷你 19/19、设计系统 12/12、列表交互 5/5，关联 CTest 7/7。日志位于 `build/qa/`，分离 QML 明细为 `build/release/qml-vocal-separation.txt`。

已检查 `ui-refinement-professional.png`、`ui-refinement-mini-final.png` 和 `ui-refinement-tag.png` 的受影响布局。专业最终截图使用测试夹具，只证明控件几何，不证明真实音频波形/听感。没有递归布局警告；保留已有歌词夹具 undefined-to-int 警告及控制器测试工作进程退出警告。下载并行通过本地 HTTP 传输及测试 worker 验证，未重新进行大型真实模型 GPU 推理、外网下载、全部 DPI/主题验收或全产品回归；本节不代表新安装包已生成。
