...
class BundlePolicyTests(BundleFixture):
    ...
    def test_removed_score_components_rejected(self):
        for name in ("AgTranscriptionWorker", "verovio-data", "yourmt3-model", "basic_pitch.json", "transcription-capabilities.json"):
            with self.subTest(name=name):
                artifact = self.resources / name
                artifact.write_bytes(b"old score component")
                with self.assertRaisesRegex(packaging.PackageError, "Removed audio-to-MIDI/score"):
                    packaging.validate_bundle(self.app)
                artifact.unlink()

    def test_old_player_with_score_controller_rejected(self):
        player = self.app / "Contents/MacOS/AgPlayer"
        player.write_bytes(player.read_bytes() + b"TranscriptionController")
        with self.assertRaisesRegex(packaging.PackageError, "rebuild from stripped sources"):
            packaging.validate_bundle(self.app)
