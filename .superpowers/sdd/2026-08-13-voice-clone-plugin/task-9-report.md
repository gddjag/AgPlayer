...
- Every file URL is derived exactly from its declared official repository, immutable revision, and upstream path. Index auxiliary files carry their own pinned official per-file source identity; community repositories, altered revisions/paths, lookalike hosts, userinfo, and custom ports are rejected.
- Loopback acceptance no longer depends on `BUILD_TESTING`: every production parser/manager/controller defaults to `OfficialOnly`; only an explicitly injected test policy accepts loopback. The Debug test build asserts the default still rejects loopback.
- Each shipped package carries a canonical `fileGraphSha256`. C++ independently rebuilds it from model metadata, the complete sorted license set, and every normalized `(source repository, revision, upstream path, local path, SHA-256, size)` tuple, then compares it with a compiled package-ID allowlist. Recomputed attacker values, truncation, additions, substitutions, and path changes cannot self-authorize through JSON.
...
- Download UI state is bound to the package manager's actual `downloadModelId`, never the currently selected card. Downloading exposes pause/cancel, paused exposes resume/cancel, and failure exposes the synchronized error plus retry.
...
- Fix2 RED/GREEN: build-wide loopback permission, JSON-self-asserted graph completeness, and selection-scoped progress were replaced by explicit test-only policy injection, compiled graph digests with per-model tamper/truncation tests, and controller-owned download identity/control/error state.
...
qml_voice_clone_test ................. Passed (11/11)
