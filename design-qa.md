...
- 当前实现：`docs/qa/evidence/integrated-theme/integrated-1672x941-single-waveform-v4.png`
- 同画布合成：`docs/qa/evidence/integrated-theme/reference-vs-current-1672x941-single-waveform-v4.png`（左参考、右实现）
- 运行日志：`docs/qa/evidence/integrated-theme/integrated-1672x941-single-waveform-v4-runtime.log`
...
7. 移除 Integrated 已播放波形的矩形裁切叠图，由同一个 `WaveformItem` 直接着色播放进度，消除高振幅处的裁切竖边；波形与播放栏间距进一步统一为上下各 4px，底栏增高到 91px。
...
- 波形：列表与波形仅保留 4px 间距；单画布直接绘制播放进度，不再使用会形成竖边的裁切叠图；悬停时间、毛玻璃时长/拖出胶囊、2px Handle 和 7%–28% 透明度的底部可拖导航条均完整。
