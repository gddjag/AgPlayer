...
{
    return value == 1 || value == kCustomColorChoiceMode
        ? value : kDefaultColorChoiceMode;
}

QString defaultThemePresetId()
{
    return QStringLiteral("systemBlue");
}
...
QString normalizedThemePreset(const QString& value)
{
    ...
    return ThemeManager::legacyPresetSeed(value).has_value()
        ? value : defaultThemePresetId();
}
...
QString themePresetColor(const QString& value)
{
    ...
    if (match != presets.cend()) {
        return match->stops[1].name(QColor::HexRgb).toUpper();
    }
    return ThemeManager::legacyPresetSeed(value)
        .value_or(ThemeManager::defaultSeed())
